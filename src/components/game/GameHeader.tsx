...
export function GameHeader({ title, opponent = { name: 'Соперник Робот' }, onInvite, badge, leading, trailing, className }: GameHeaderProps) {
  ...
  const headerTrailing = trailing ?? (onInvite ? <Link className="game-header__exit button button--surface button--icon" href="/?leave=1" aria-label="Вернуться на главную" onClick={() => telegram.impact('light')}>

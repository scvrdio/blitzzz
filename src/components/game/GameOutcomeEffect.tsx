...
export function GameOutcomeEffect({ outcome }: { outcome: GameOutcome | null }) {
  ...
  useEffect(() => {
    ...
    const fire = () => {
      ...
      confetti({ particleCount: 1, angle: 60, spread: 55, startVelocity: 60, gravity: 1.8, origin: { x: 0, y: 1 }, colors: ['#004cff'], disableForReducedMotion: true });
      confetti({ particleCount: 1, angle: 120, spread: 55, startVelocity: 60, gravity: 1.8, origin: { x: 1, y: 1 }, colors: ['#000000'], disableForReducedMotion: true });
